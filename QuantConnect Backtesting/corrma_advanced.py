...
'''
"Advanced" algorithm beyond the corrma_basic. The idea is to not only swap to the dependent when < corr_threshold.
Now, it also swaps BACK to the independent if the corrma goes below 0
But, with ETH and BTC, this is essentially doomed to fail (or at least be worse than just hold until > threshold),
because ETH has better price action than BTC!
'''
class CorrmaAlgoAdvanced(QCAlgorithm):
    ...
    def OnData(self, data: Slice):
        ...
        if self.corrma.IsReady and self.EndDate - self.Time > timedelta(1):
            ...
            if value <= self.corrma_ratio and value > 0:
                # Swap to Dependent
                if not self.holding_dependent:
                    self.holding_dependent = True
                    self.Liquidate(self.ind_symbol)
                    self.SetHoldings(self.dep_symbol, 1)  
            else:
                # Swap to Independent
                if self.holding_dependent:
                    self.holding_dependent = False
